...
def undistort_frame(frame, camera_matrix, dist_coeffs, roi, new_camera_matrix):
...
def merge_hough_lines(lines, threshold_angle=3):
    """
...
#         print(merged_lines)
        line_img, poly_vertices , solid, div = new_slope_lines(line_img,merged_lines)
    except:
        # In GTA unComment the following
#         rAll()
        cv2.putText(line_img, "No lanes", (200,50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
    return line_img, poly_vertices, solid, div
...
def weighted_img(img, initial_img, α=0.1, β=1., γ=0.):
    ...
    """
    # Debugging: print shapes and channels of the images
    # print(f"Initial img shape: {img.shape}, Initial image shape: {initial_img.shape}")
    
    # Ensure img and initial_img have the same height and width
    ...
    # Handle the case where initial_img has an alpha channel
    if initial_img.shape[2] == 4:
        initial_img = cv2.cvtColor(initial_img, cv2.COLOR_BGRA2BGR)
        # print(f"Converted initial_img from BGRA to BGR: {initial_img.shape}")
    
    # Handle the case where img has an alpha channel
    if img.shape[2] == 4:
        img = cv2.cvtColor(img, cv2.COLOR_BGRA2BGR)
        # print(f"Converted img from BGRA to BGR: {img.shape}")
    
    # Ensure both images have the same number of channels
    if len(img.shape) == 2:  # If img is grayscale, convert to BGR
        img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
        # print(f"Converted img to BGR: {img.shape}")
        
    if len(initial_img.shape) == 2:  # If initial_img is grayscale, convert to BGR
        initial_img = cv2.cvtColor(initial_img, cv2.COLOR_GRAY2BGR)
        # print(f"Converted initial_img to BGR: {initial_img.shape}")

    # Final check before combining images
    if img.shape[2] != initial_img.shape[2]:
        raise ValueError(f"The number of channels in `img` ({img.shape[2]}) and `initial_img` ({initial_img.shape[2]}) must be the same")
...
def lane_finding_pipeline(image, s_thresh=(140, 180), sx_thresh=(10, 150), kernel_size=5,
                          h_threshold=40, h_theta=1 * np.pi / 180,
                          min_line_len=10, max_line_gap=1, poly_vertices=[]):
    # Grayscale
    ...
    # compareGray(image, canny_img, 'Original', 'S and SobelX')
    if poly_vertices:
    ...
    # cv2.imshow('Masked_img', masked_img_normalized)
    
    # Hough Transform Lines
    # compareGray(canny_img, masked_img, 'S and SobelX', 'Masked')
    houghed_lines, vert, solid, div = hough_lines(img=smoothed_img, rho=1, theta=h_theta, threshold=h_threshold, min_line_len=min_line_len, max_line_gap=max_line_gap)
...
#     print(f"length :{len(touching_lines)}")
    ...
    if(len(touching_lines) >= 2):
        # cv2.putText(img, f"{len(touching_lines)} different non touching lanes", (50,150), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 2)
        slopes = []
        if old :
            slopes = touching_lines
            # draw_lines(xy_from_slope(slopes[0],rows),img,generate_color(5))
        else :
...
def new_slope_lines (image, lines):
    ...
    if(left_lines):
        # state = 'Solid' if len(left_lines) < 2 else 'Dotted'
        # cv2.putText(img, f'{state} Left', (200,30), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 0, 255), 2)
...
#         touching_lines.extend([xy_from_slope(last_left_line,rows)])
    if(right_lines):
        solid = len(right_lines) < 2 
        state = 'Solid' if solid  else 'Dotted'
        cv2.putText(img, f'{state} Right', (300,30), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 0, 255), 2)
...
#         touching_lines.extend([xy_from_slope(last_right_line,rows)])
    poly_vertices, div = detect_lanes([left_line,right_line], img, True)
   
    return cv2.addWeighted(image,0.7,img,0.4,0.), poly_vertices, solid, div
 
def construct_frame(lane_status, deviation):
    # Lane status: 1 bit (1 if solid, 0 if dotted)
    lane_status_bit = 1 if lane_status else 0
    # print(f"Lane Status: {lane_status}, Lane Status Bit: {lane_status_bit}")
    
    # Sign bit: 1 bit (1 if positive, 0 if negative)
    sign_bit = 1 if deviation >= 0 else 0
    # print(f"Deviation: {deviation}, Sign Bit: {sign_bit}")
    # 
    # Normalized deviation: already provided as a 4-bit value (0-15)
    normalized_deviation = abs(int(deviation * 16))
    if normalized_deviation > 15:
        normalized_deviation = 15
    # print(f"Normalized Deviation: {normalized_deviation}")
    
    # Combine into a single 6-bit value
    frame = (lane_status_bit << 5) | (sign_bit << 4) | normalized_deviation
    # print(f"Frame: {bin(frame)[2:].zfill(6)}")
    
    return frame
########################################## Main Functions Definitions ##########################################
def transmit_data(lane_output,alarm_flag, emergency_flag): 
    ser = serial.Serial('COM4', 9600, timeout=1)
    print("Serial port opened")
    time.sleep(2) # Wait for the system to stabilize
    while True:
        if alarm_flag.value == True and emergency_flag.value == False:
            eye = "10"
        elif alarm_flag.value == True and emergency_flag.value == True:
            eye = "11"
        else:
            eye = "00"
        lane = str(lane_output.value)
        binary_data = lane + eye
        #print("Binary data", binary_data) 
        data = int(binary_data,2)
        ser.write(bytes([data]))
        print("Data transmitted", bytes([data]))
        ser.flush()
        time.sleep(0.2)
    return

def Lane_Detection(s_thresh_min_var, s_thresh_max_var, sx_thresh_min_var, sx_thresh_max_var, kernel_size_var, h_threshold_var, h_theta_var, min_line_len_var, max_line_gap_var,lane_output):
    global last_good_left, last_good_right, last_left_line, last_right_line, times, ltimes
    for i in range(2, 0, -1):
        print(i)
        time.sleep(1)
    with np.load('D:\GraduationProject\Inference_tools_python\calibration_data.npz') as data:
        ret = data['ret']
    ...
    cap = cv2.VideoCapture("D:\GraduationProject\Inference_tools_python\Webcam Check _ Webcam Test - Google Chrome 2024-06-25 20-45-53.mp4")  # 0 is usually the default camera
    ...
    while True:
        ...
        # last_time = time.time()
        screen_undistorted = undistort_frame(screen, camera_matrix, dist_coeffs,roi, new_camera_matrix)
        op, vert, solid, div= lane_finding_pipeline(screen_undistorted)
        if vert:
            flagged = True
            counter = 0
            last_vert = vert
...
#                     print("Hard")
        ...
        cv2.imshow('window', cv2.cvtColor(op, cv2.COLOR_BGR2RGB))
        lane_output.value = bin(construct_frame(solid, div*3))[2:].zfill(6)
        #print(f"Output: {lane_output.value}")
        if cv2.waitKey(25) & 0xFF == ord('q'):
            cv2.destroyAllWindows()
            break
    return lane_output

def eye_score(alarm_flag, emergency_flag, host_ip, port):
    print("Eye score is running")
    ...
    N = 100  # Define the period for EMA
    ...
    while True:
        ...
        if ema < 0.65:
            if alarm_time is None:
                ...
                continue
        elif ema > 0.75 and alarm_flag.value == True and emergency_flag.value == False :
            alarm_flag.value = False
            alarm_time = None
            print("Alarm flag is reset")
        else:
            continue
    return
...
if __name__ == '__main__':
    ...
    alarm_flag = manager.Value('b', False)
    emergency_flag = manager.Value('b', False)
    UDP_state = manager.Value('b', False)
    lane_output = manager.Value('s', '000000') 
    host_ip = '192.168.1.4' 
    port = 9999
    s_thresh_min_var, s_thresh_max_var, sx_thresh_min_var, sx_thresh_max_var, kernel_size_var, h_threshold_var, h_theta_var, min_line_len_var, max_line_gap_var=140, 180, 10, 150, 5, 40, 2*np.pi / 180, 50, 1
    while True:
        p0 = multiprocessing.Process(target=eye_score, args=(alarm_flag, emergency_flag, host_ip, port))
        p1 = multiprocessing.Process(target=Lane_Detection, args=(s_thresh_min_var, s_thresh_max_var, sx_thresh_min_var, sx_thresh_max_var, kernel_size_var, h_threshold_var, h_theta_var, min_line_len_var, max_line_gap_var,lane_output))
        p2 = multiprocessing.Process(target=transmit_data, args=(lane_output,alarm_flag,emergency_flag))
        p0.start()
        p1.start()
        p2.start()
        p0.join()
