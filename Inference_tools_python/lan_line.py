...
# %%
# Load the previously saved camera matrix and distortion coefficients
with np.load('Inference_tools_python\calibration_data.npz') as data:
    ret = data['ret']
...
cap = cv2.VideoCapture("Inference_tools_python\Webcam Check _ Webcam Test - Google Chrome 2024-06-25 20-45-53.mp4")  # 0 is usually the default camera
...
# Undistort function for live stream
...
def start_lane_detection(s_thresh_min_var, s_thresh_max_var, sx_thresh_min_var, sx_thresh_max_var, kernel_size_var, h_threshold_var, h_theta_var, min_line_len_var, max_line_gap_var):
    global last_good_left, last_good_right, last_left_line, last_right_line,ltimes, times
    ...
    while True:
        ...
        # last_time = time.time()
        screen_undistorted = undistort_frame(screen)
...
#                     print("Hard")
        ...
        output = bin(construct_frame(solid, div*3))[2:].zfill(6)
...
start_lane_detection(140, 180, 10, 150, 5, 40, 2*np.pi / 180, 50, 1)
